...
from classDemo.myMongoDB import NewMongDB
...
def get_cookie(user,pwd):
    ...
    if result['msg'] == '登录成功' or result['msg'] == '非生产环境,不做校验！':
        cookies = requests.utils.dict_from_cookiejar(response.cookies)
        NewConfigparser('cookieInfo').set_conf(cookie=cookies)
        return '登录成功'

    elif '密码错误' in result['msg']:
        msg = result['msg']
        return result['msg']
    else:
        needClient = True
    ...
    if needClient:
        ...
        if result['msg'] != 'ok' and u'验证码发送过于频繁' not in result['msg']:
            msg = result['msg']
            # base.consoleLog('接口异常！\n接口地址：%s\n请求参数：%s\n返回结果：%s' % (url, data, msg),'w')
        ...
        data = {
            ...
            'user_phone': user,
            'user_pwd': pwd,
            'verificationCode': NewMongDB().db_find()
        }
        headers = {
            'content-type': 'application/json',
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/57.0.2987.110 Safari/537.36'
        ...
        if result['msg'] == 'ok':
            cookies = requests.utils.dict_from_cookiejar(response.cookies)
            # base.set_conf('cookieInfo', 'cookies', cookies)
            return '线上环境登录成功'
        else:
            msg = result['msg']
            # base.consoleLog('接口异常！\n接口地址：%s\n请求参数：%s\n返回结果：%s' % (url, data, msg),'w')
